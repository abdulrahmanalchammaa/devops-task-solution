...
- **Container 2 — Web server**: PHP 7.1+ with the proper driver to connect to the MSSQL container, serving [`QuickDbTest.php`](web-root/QuickDbTest.php) from the web root.
- **Launcher**: builds and starts everything in one step.

## Solution

Implemented as three services in [`docker-compose.yml`](docker-compose.yml):
